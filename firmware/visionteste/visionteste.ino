...
constexpr unsigned long ULTRASONIC_PULSE_TIMEOUT_US = 25000;
...
enum class LocalFallbackStage : uint8_t {
  READY_FOR_PRESENT = 0,
  READY_FOR_ACTION = 1,
  COMPLETED = 2,
};

LocalFallbackStage localFallbackStage = LocalFallbackStage::READY_FOR_PRESENT;
...
void handleUltrasonicFallback() {
  ...
  if (ultrasonicPresenceLatched) {
    return;
  }
  ultrasonicPresenceLatched = true;

  if (localFallbackStage == LocalFallbackStage::READY_FOR_PRESENT) {
    Serial.println("ULTRA_TRIGGER_PRESENT");
    isPresenting = true;
    runPresentation();
    isPresenting = false;
    localFallbackStage = LocalFallbackStage::READY_FOR_ACTION;
    return;
  }

  if (localFallbackStage == LocalFallbackStage::READY_FOR_ACTION) {
    Serial.println("ULTRA_TRIGGER_ACTION");
    isPresenting = true;
    runAction();
    isPresenting = false;
    localFallbackStage = LocalFallbackStage::COMPLETED;
    return;
  }

  Serial.println("ULTRA_IGNORED");
}
